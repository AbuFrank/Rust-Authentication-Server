...
pub async fn login_handler(users: Users, body: LoginRequest) -> WebResult<impl Reply> {
    match users.iter().find(|(_uid, user)| user.email == body.email && user.pw == body.pw) {
        Some((uid, user)) => {
            let token = auth
                ::create_jwt(&uid, &Role::from_str(&user.role))
...
fn init_users() -> HashMap<String, User> {
    ...
    map.insert(String::from("1"), User {
        uid: String::from("1"),
        email: String::from("user@example.com"),
        pw: String::from("1234"),
        role: String::from("User"),
    });
    map.insert(String::from("2"), User {
        uid: String::from("2"),
